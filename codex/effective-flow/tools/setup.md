...
Wenn mehrere ADR-Verzeichnisse existieren und keines eindeutig `docs/adr/` ist: Frage den User: **In welchem Verzeichnis soll die Effective Flow-Projektsetup-ADR liegen?**
- docs/adr/ -- Empfohlener Default für die Projektsetup-ADR
- docs/decisions/ -- Vorhandenes Verzeichnis nutzen
- adr/ -- Vorhandenes Verzeichnis nutzen
...
Frage den User: **Wie möchtest du die Effective Flow-Konfiguration einrichten?**
- Express -- Sichere Defaults übernehmen (bei vorhandener Config deren aktuelle Werte behalten) — ein Bestätigungsschritt, dann fertig
- Geführt -- Schritt für Schritt durch die Optionen — jede wird erklärt, ideal wenn du Effective Flow noch nicht kennst
...
Frage den User: **Soll die Umsetzung in einem separaten Git-Worktree laufen?**
- Ja -- worktree.enabled = true (Default) — Umsetzung läuft in einem separaten Worktree mit eigenem Liefer-Branch
- Nein -- worktree.enabled = false — In-Place ohne Worktree; Liefer-Branches werden bei Bedarf im Haupt-Repo erzeugt
...
Frage den User: **Welche Abschluss-Aktion soll Effective Flow standardmäßig nutzen?**
- Merge -- delivery.completion = merge (Default) — Branch lokal in den Basis-Branch mergen, ohne PR
- Pull-Request -- delivery.completion = pr
- Nur Branch -- delivery.completion = branch
- Beim Lauf fragen -- delivery.completion = null — die Aktion wird pro Lauf erfragt
...
Frage den User: **In welcher Sprache sollen die Statusmarker neuer Plan-Dateien stehen?**
- Englisch -- plan.markerLanguage = en (Default, falls keine Sprache aus vorhandenen Plänen erkennbar ist)
- Deutsch -- plan.markerLanguage = de
...
- Lokal -- tracker.mode = local (Default) — Markdown-Report unter .effective-flow/review/
...
Frage den User: **Möchtest du erweiterte Einstellungen (Review, Apply-Review, Pfade, Feinheiten) anpassen?**
- Nein -- Sichere Defaults bzw. bestehende Werte behalten — empfohlen, wenn du Effective Flow noch kennenlernst
- Ja -- Die restlichen Optionen einzeln durchgehen, jede erklärt
...
- Starte keine Projektvalidation; Linting, Tests und Build-Checks sind Aufgabe anderer Skills wie ``code-validator``.
