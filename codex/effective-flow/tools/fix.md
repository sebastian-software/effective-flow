...
1. Versuche den Bug zu reproduzieren:
   - ``code-validator`` für aktuellen technischen Zustand
   - falls möglich: ``test-writer`` für einen fehlschlagenden Test, der das Verhalten dokumentiert
...
Frage den User: **Diagnose und Fix-Strategie freigegeben?**
...
1. Starte den passenden Implementer-Skill:
   - ``ui-implementer``, ``nodejs-implementer``, ``rust-implementer`` oder ``generic-implementer``
