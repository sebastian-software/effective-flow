...
- Root `README.md` as the marketing entry point of the standard doc structure: ``marketing-writer``
- User and project documentation (incl. user docs under `docs/user-guide/` and technical docs under `docs/developer-guide/`): ``docs-writer``
- In-code documentation, JSDoc/TSDoc, CLI help texts: ``code-documenter``
- Technical check for generated artifacts, CLI help, build files or code files: ``code-validator``
...
- In **one** run, create the three roles of the standard structure and coordinate the agents so the two README links point to existing targets at the end: ``marketing-writer`` for the root `README.md`, ``docs-writer`` for `docs/user-guide/README.md` (plus initial guides) and `docs/developer-guide/README.md`.
...
- if the target path points to an existing file: clarify replacement or a new slug with the user before ``docs-writer`` starts
...
3. Determine the doc category per `Doc categories`:
   ...
   - for the marketing entry point (root `README.md`) the category is omitted: it is not one of the four `docs/` categories, the target path is `README.md` and the implementation goes to ``marketing-writer``
...
Frage den User: **Documentation plan approved?**
...
2. Start the appropriate agent:
   - ``marketing-writer`` for the root `README.md` as the marketing entry point
   - ``docs-writer`` for category guides, category entry-point READMEs (e.g. `docs/user-guide/README.md`, `docs/developer-guide/README.md`), API/CLI docs, migration, changelog and skill documentation – **not** for the root marketing README
   - ``code-documenter`` for JSDoc/TSDoc, inline comments and CLI help texts in code files
...
4. Start ``code-validator`` when doc changes affect technical artifacts or the project build can plausibly check the change.
