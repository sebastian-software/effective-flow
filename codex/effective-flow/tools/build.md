...
- Feature -- Neue Funktionalität, neues UI-Element, neue Seite oder Integration
- Bugfix -- Fehler beheben, unerwartetes Verhalten korrigieren
- Refactoring -- Code umstrukturieren ohne Verhaltensänderung
- Dokumentation -- Dokumentation ändern ohne Produkt- oder Codeverhalten
...
- starte ``generic-implementer`` zusätzlich, wenn CI, Tooling, Konfiguration, Dependency-Manifeste oder sonstige generische Artefakte betroffen sind
...
- Code-Doku: ``code-documenter``
- User-Doku: ``docs-writer``
- Tests: ``test-writer``
- E2E: ``e2e-tester``
- Validierung: ``code-validator``
...
Frage den User: **Implementierungsplan freigegeben?**
...
1. Starte den passenden Implementer-Skill mit dem abgestimmten Plan:
   - Frontend: `Verwende den Skill `ui-implementer` für diese Phase.`
   - Backend/CLI: `Verwende den Skill `nodejs-implementer` für diese Phase.`
   - Rust: `Verwende den Skill `rust-implementer` für diese Phase.`
   - Generic/Tooling/CI/Config: `Verwende den Skill `generic-implementer` für diese Phase.`
...
1. ``code-documenter`` für In-Code-Dokumentation aller neuen oder geänderten Exports – JSDoc/TSDoc bei JS/TS, rustdoc-Doc-Comments (`///`/`//!`) bei Rust
2. ``docs-writer`` für README/Guide-Updates, falls die Änderung nutzerrelevant ist (bei Rust inkl. Crate-/Modul-Doku)
...
1. ``test-writer`` für Unit-Tests und Komponententests
2. ``e2e-tester`` für neue User-Flows, falls ein echter Flow dazugekommen ist

### Phase 5: Validierung

1. Starte ``code-validator``.
