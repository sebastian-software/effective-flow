...
Reicht die Information für eine autonome Umsetzung nicht aus, wird das Issue **übersprungen**, mit Label `sf-needs-planning` markiert und per Kommentar erklärt. `{{SKILL:plan-issue}}` sammelt diese Issues später ein und vervollständigt die Planung.
...
- übersprungene Issues (`sf-needs-planning`) mit Grund und dem Hinweis, dass `{{SKILL:plan-issue}}` die Planung vervollständigen kann
