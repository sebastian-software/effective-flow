...
`wontfix` already exists on many trackers; the helper creates it only if it is missing. `effective-flow-issue-done` and `effective-flow-needs-planning` belong to the issue-driven flow (`{{SKILL:apply-issues}}`/`{{SKILL:plan-issue}}`) and are created idempotently there.
