...
when: residual implementation-review candidates are about to be classified before ID reservation or materialization
```
...
- `{{SKILL:apply-review}}` can process only the admitted findings later in the familiar format.
...
- All tables and finding blocks stay in the `{{SKILL:review}}` format, with one additional
