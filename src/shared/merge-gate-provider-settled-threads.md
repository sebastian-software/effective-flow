...
This fragment defines a **provider-settled** review thread for `{{SKILL:merge-gate}}`, its only
...
`{{SKILL:iterate}}` again and again. The reviewer's own later approval, read from forge state,
...
key already holds `implemented` from this run to `{{SKILL:iterate}}`. The second half closes the
