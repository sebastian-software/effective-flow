...
and stay unlocalized in both forms, as do the forge issue-reference keywords — the auto-close
keyword with its variants (`Closes #<issue>`) and the non-closing `Refs #<issue>` — which the code
host parses and which therefore stay English in a PR body written in another language. `Action`,
