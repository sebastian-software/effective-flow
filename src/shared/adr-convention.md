...
Firmo führt Architekturentscheidungen (ADRs) als **lebende Dokumente**: mutable
...
Baustein ist die maßgebliche Konvention für alle **von Firmo erzeugten** ADRs.
...
- **Dateiname:** nummernlos, kebab-case-Slug — `docs/adr/<slug>.md` (z. B.
  `docs/adr/firmo-project-setup.md`).
...
_nummeriert_, _eine-Entscheidung-pro-Record_ und _nur Rationale, keine Config-Werte_. Firmo
weicht hier **bewusst** ab; diese Konvention hat für Firmo-erzeugte ADRs Vorrang. Begründung
je Divergenzpunkt:

- **Mutabilität (lebend statt Supersede-Kette).** Firmo optimiert auf kleinen, eindeutigen
...
den Skill für Firmo-Agents und -Tools gezielt über die `skills`-Config (`include`/`exclude`,
auch per-Agent/-Tool) zu- oder abschalten. Firmos eigene ADR-Konvention bleibt davon
unberührt.
