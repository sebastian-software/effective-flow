...
`{{SKILL:merge-gate}}`'s merge phase and its post-merge phase, which is why that tool defers this
...
stays in the "PR review comment integration" building block, which `{{SKILL:merge-gate}}` loads
