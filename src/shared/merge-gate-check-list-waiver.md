...
`{{SKILL:merge-gate}}`: the rules that decide whether the waiver question is posed, how each of
...
runs deciding from text they have not loaded. `{{SKILL:merge-gate}}` is this fragment's only
