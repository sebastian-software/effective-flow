...
2. Erlaubte Formen:
   ...
   - Nummer, z. B. `0030`
3. Wenn keine Datei passt: melde den Fehler und nenne, dass `{{SKILL:sf-open-plans}}` offene Pläne auflisten kann.
...
2. Bestimme die Empfehlung:
   - Feature oder `{{SKILL:sf-build}}` → `{{SKILL:sf-build}}`
   - Bugfix oder `{{SKILL:sf-fix}}` → `{{SKILL:sf-fix}}`
   - Refactoring oder `{{SKILL:sf-refactor}}` → `{{SKILL:sf-refactor}}`
   - Dokumentation oder `{{SKILL:sf-docs}}` → `{{SKILL:sf-docs}}`
3. Wenn der aktuelle Skill `{{SKILL:sf-apply-plan}}` ist: verwende die Empfehlung als Ziel-Workflow und fahre fort.
