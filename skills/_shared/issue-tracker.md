...
Bestimme im Remote-Modus das Werkzeug analog zu `{{SKILL:sf-pr}}`:
...
2. **Werkzeug wählen:**
   ...
   - sonst (`auto`): Lies die `origin`-URL (`git remote get-url origin`). Enthält der Host `github.com`, ist das Werkzeug `gh`; andernfalls wird Forgejo angenommen und `tea` verwendet.
...
Ein Finding-Issue muss **self-contained** sein: eine fremde LLM-Session muss es ohne Zugriff auf die erzeugende Session abarbeiten können. Es enthält dieselben inhaltlichen Felder wie ein Finding-Block des lokalen Report-Formats (siehe `{{SKILL:sf-review}}`, „Bericht-Format").
