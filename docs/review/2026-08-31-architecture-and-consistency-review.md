...
| §5 behavioural eval layer                                      | partial     | `364f4d0` (#399) added `guard-blocks-merge` and `merge-proceeds`, `1d8a0ed` (#407) `linked-issue-open-points`, `4321151` (#417) `unreported-checks-block-merge`, and the F-15 delivery added configured-reviewer coverage; production `iterate` classification still has none |
| F-15 `merge-gate` size                                         | implemented | #400–#402 and #419 deferred four local routes; the final configured-reviewer extraction reduced the maximum always-loaded core from 2,746 to 2,127 lines while preserving the sequential gate shell and withdrawing the unsupported ~1,200-line target                        |
