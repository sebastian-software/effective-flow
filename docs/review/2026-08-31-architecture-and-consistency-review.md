...
| F-07, F-08 retired `worktree.*` delivery keys, `prReview.*`    | implemented | #420 — `worktree.baseBranch`, `worktree.branchPrefix`, `worktree.completion` and `prReview.*` are retired: a run stops on one whose successor is absent and names `setup`, which rewrites it in place                                                                                                                                                                                                                                        |
| F-09, F-11 no scenario-level entry point for the three lines   | implemented | `473e518` (#424) — `setup` now opens with a `Chat` and a `Profile` ask offering the three topologies as named profiles, and the user guide presents them in `getting-started.md`, `tools-setup.md` and `configuration.md`                                                                                                                                                                                                                    |
| §5 behavioural eval layer                                      | partial     | Six `merge-gate` scenarios: `364f4d0` (#399) `guard-blocks-merge` and `merge-proceeds`, `1d8a0ed` (#407) `linked-issue-open-points`, `4321151` (#417) `unreported-checks-block-merge`, `2c39546` (#427) `unreported-checks-at-phase-four`, `7126b0c` (#425) `configured-reviewer-set-aside-blocks`, whose runs additionally pin the Phase-3 delegation handoff through an `iterate` echo; production `iterate` classification still has none |
...
| M-3 `commit-message-rules` move candidate                      | withdrawn   | #454 — not moved: the fragment stays in Effective Flow, with `effective-delivery` declared authoritative for commit-message craft and the runtime-effect and squash-title rules named as overrides                                                                                                                                                                                                                                           |
| M-4 `investigation-method` move candidate                      | implemented | #448 — not moved: the fragment stays in Effective Flow, with `effective-delivery` declared authoritative for diagnostic depth                                                                                                                                                                                                                                                                                                                |
| M-5 `adr-convention` + `project-adr-convention` move candidate | withdrawn   | #454 — not moved: `adr-convention` is the declaration `effective-product` requires before a living record is used, and `project-adr-convention` is the resolution mechanism the skill leaves to the deferring side                                                                                                                                                                                                                           |
...
- **The §6.2 move recommendation did not survive a per-item comparison.** §6.2, repeated in P4,
  ...
  (`adr-convention` + `project-adr-convention`) are resolved inside Effective Flow rather than
  offered upstream, and for opposite reasons. M-3 was the closest of the six to a second copy:
  `effective-delivery/references/evidence-and-delivery.md` already covers the Conventional-Commit
  fallback, type choice from the actual change, subjects and bodies, so the fragment is thinned to
  the two rules the skill does not state — runtime effect and the squash PR title — plus this
  repository's unconditional bans and one labelled minimal fallback. M-5 is the reverse: the
  skill's ADR reference permits a living lifecycle only where the repository declares five things
  first, so `adr-convention` is that declaration and thinning it would delete what the skill asks
  for; and `project-adr-convention` answers a question the owner leaves open, because a skill that
  follows the repository's declared convention needs the deferring side to determine it. This
  review's own premise for M-5 is therefore wrong in its second half: "it says it follows the
  repository's declared convention without specifying how to determine it" is the reason to keep
  the resolver, not to move it. M-4 (`investigation-method`) is confirmed and implemented: `effective-delivery` is
...
| M-3 | `src/shared/commit-message-rules.md` + the Conventional-Commit type-by-effect rule | 15 + related prose in `commit.md` | `effective-delivery`                                                       | **Withdrawn — resolved locally.** `evidence-and-delivery.md` already states the Conventional-Commit fallback, type choice from the actual change, subject and body rules and the `chore`-escape-hatch prohibition, so the skill is declared authoritative and the duplicated prose is removed. What stays is what upstream does not state: type by effect for deployment-effective configuration, the squash PR title as the release signal, this repository's unconditional attribution bans where upstream's are conditional, and one labelled minimal fallback.                                                                                          |
...
| M-5 | `src/shared/adr-convention.md` + `project-adr-convention.md`                       |                         110 + 220 | `effective-product` (`references/adr-format.md` exists)                    | **Withdrawn — resolved locally, and this row's premise was wrong.** `adr-format.md` permits a living lifecycle only where the repository declares five things first, so `adr-convention.md` is the declaration the skill demands rather than a competing playbook; it now states all five and the skill is named authoritative. `project-adr-convention.md` supplies the determination procedure the skill leaves to the deferring side: deferring to the repository's declared convention presupposes a side that resolves it, and Effective Flow writes ADRs into arbitrary target projects, so that side is Effective Flow. Resolution is orchestration. |
...
M-1 was the largest of the six at ~406 lines, and was withdrawn on the evidence above. With M-5
also resolved inside Effective Flow, M-6 is the only row still open, and its own upstream evidence
was withdrawn — so this section no longer proposes a move.
...
- **M-1 … M-5** are closed without a move; **M-6** alone stays open, pending a re-assessment of the upstream evidence withdrawn in `fcffae7` (#244).
