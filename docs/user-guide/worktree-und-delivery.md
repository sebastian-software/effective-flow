...
Sobald ein Effective Flow-Tool Code, Tests oder Dokumentation ändert (`build`, `fix`, `refactor`,
...
`.effective-flow/.worktrees`) und wird per `git worktree add` zusammen mit dem Liefer-Branch angelegt.
...
`<delivery.branchPrefix>/<skill>/<slug>` (z. B. `effective-flow/build/user-login`), abgeleitet aus
Plan-Titel, Aufgabenbeschreibung, Issue oder Finding; bei einer Namenskollision hängt Effective Flow
...
Remote-Ref, holt Effective Flow vorher per `git fetch` den aktuellen Stand, damit der Liefer-Branch
...
gewünscht, erzeugt Effective Flow den Liefer-Branch direkt im Haupt-Repo statt in einem Worktree.
Enthält dein aktueller Arbeitsbaum dann uncommittete Änderungen, die nicht Teil des
Liefer-Branches werden sollen, fragt Effective Flow nach, statt sie still zu stagen, zu stashen oder
zu überschreiben.

### Was committet wird und was lokal bleibt

Von den Effective Flow-Artefakten wird **ausschließlich die Plan-Datei** committet – und auch das
nur, sofern der Workflow eine geführt hat. Alle übrigen `.effective-flow/`-Artefakte
...
| Wert     | Verhalten                                                                                                                                                                                                 |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `merge`  | Branch wird lokal per Fast-Forward oder Merge-Commit in `delivery.baseBranch` gemergt. Bei Konflikt stoppt Effective Flow, belässt den Branch und informiert dich – keine automatische Konfliktauflösung. |
| `pr`     | Branch wird gepusht, [`/effective-flow pr`](./tools-einbringen.md) öffnet einen Pull-Request gegen `delivery.baseBranch`.                                                                                 |
| `branch` | Der Branch bleibt einfach im lokalen Repo liegen; du entscheidest später selbst über PR oder Merge.                                                                                                       |

Ist `delivery.completion` nicht gesetzt (`null`), fragt Effective Flow bei jedem Lauf erneut nach der
gewünschten Aktion.

Ist ein Worktree beteiligt, entfernt Effective Flow ihn nach erfolgreichem Abschluss automatisch
(`git worktree remove`); der Liefer-Branch selbst bleibt im lokalen Repo erhalten. Schlägt
das Entfernen wegen uncommitteter Reste fehl, bleibt der Worktree bestehen und Effective Flow meldet
den Pfad.
...
Effective Flow und meldet den Konflikt, statt die History zu überschreiben.

## Plan-Datei: Statuswechsel am Delivery-Punkt

Führte der Workflow eine Plan-Datei, markiert Effective Flow sie erst unmittelbar am Delivery-Punkt –
...
## Zusammenspiel mit `/effective-flow pr`

Bei `delivery.completion: pr` delegiert der abschließende Schritt an
[`/effective-flow pr`](./tools-einbringen.md) und übergibt Liefer- und Basis-Branch. `/effective-flow pr` selbst
...
`gh` bzw. `tea`, siehe [Remote-Tracker](./remote-tracker.md)). Rufst du `/effective-flow pr` direkt für
...
ein **eigener, unabhängiger** Mechanismus von [`/effective-flow apply`](./tools-umsetzen.md) beim
...
- [Tools einbringen](./tools-einbringen.md) – `/effective-flow commit` und `/effective-flow pr`
