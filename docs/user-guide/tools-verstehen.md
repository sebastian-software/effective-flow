...
(`.firmo/investigation/`, `<plan.dir>/` bzw. einen Issue-Kommentar); keines ändert
...
**Typischer Aufruf:** `/firmo investigate <Symptombeschreibung>`

**Ein-/Ausgabe:** Eingabe ist die Beschreibung des beobachteten Verhaltens. Ausgabe ist
`.firmo/investigation/investigation-YYYY-MM-DD-<slug>.md` mit Symptom, Reproduktion,
...
**Zusammenspiel:** Die Empfehlung routet nach `/firmo fix` (Defekt mit klarer Ursache),
`/firmo refactor` (Strukturproblem ohne Verhaltensänderung), `/firmo build` (fehlende
Funktionalität oder bewusste Verhaltensänderung) oder `/firmo docs` (reine
...
**Typischer Aufruf:** `/firmo plan <Anforderung>`
...
**Zusammenspiel:** Der fertige Plan wird später mit `/firmo build <plandatei>`,
`/firmo fix <plandatei>`, `/firmo refactor <plandatei>` oder `/firmo docs <plandatei>`
...
übersprungen, lässt er sich später über `/firmo review <plandatei>` nachholen.
...
**Typischer Aufruf:** `/firmo open-plans`
...
**Zusammenspiel:** Reiner Lesezugriff als Sprungbrett zu `/firmo build`, `/firmo fix`,
`/firmo refactor`, `/firmo docs` (je nach empfohlenem Workflow der gelisteten Pläne) oder
`/firmo review <plandatei>`.

## `/firmo plan-issue`

**Zweck:** Sammelt Issues ein, die `/firmo apply` (genauer: der interne
Issue-Umsetzungs-Workflow) wegen fehlender Informationen übersprungen und mit dem Label
`firmo-needs-planning` markiert hat. Vervollständigt die Planung je Issue interaktiv nach
derselben Klärungs-Methodik wie `/firmo plan` und schreibt das Ergebnis als strukturierten
...
**Typischer Aufruf:** `/firmo plan-issue [Issue-Referenz(en)]`

**Ein-/Ausgabe:** Ohne Argument werden alle offenen `firmo-needs-planning`-Issues zur Auswahl
...
`tracker.mode`-Umschalter gilt hier nicht. Nach Abschluss kann `/firmo apply` das nun geplante
Issue umsetzen.
