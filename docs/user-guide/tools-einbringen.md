...
**Zusammenspiel:** Wird typischerweise am Ende eines `/firmo build`-, `/firmo fix`-,
`/firmo refactor`-, `/firmo docs`- oder `/firmo maintain`-Laufs verwendet (die diese
...
(`delivery.completion: pr`), die `/firmo build`, `/firmo fix`, `/firmo refactor`,
`/firmo docs` und `/firmo maintain` am Ende ihres Delivery-/Worktree-Handbacks auslösen
