...
read-only check on every pull request and push to the source branch (`develop`).
...
The [README ownership decision](adr/compose-readme-with-mdtheme.md) records this contract.

## Source and delivery branches

Make all README changes against `develop`. `main` is a generated release artifact,
so it does not own a second source, theme configuration, or README check workflow.
The release copies the checked, themed README from `develop`, rewrites developer
guide links, and appends the delivery notice. Each release starts from the current
source output, preserving branding without accumulating footers. The README source
and contributor tooling remain on `develop`; consumers need neither mise nor mdtheme.
