...
| `codebase-improvement`     | `review`, `refactor`, `plan`, `plan-review`, `generic-product-reviewer`                                | route-when-relevant                                | Tool-level audit reasoning in `review`/`refactor` and general audit reasoning in `generic-product-reviewer` delegate evidence, impact, candidate rejection, prioritization, complexity, root cause, and scope/risk to the skill through a minimal fallback. Effective Flow keeps orchestration, routing, finding schema/IDs, confidence/scorecard gates, and report/tracker/baseline/delivery contracts. Specialist reviewers retain their line-level checks. `plan`/`plan-review` additionally delegate generic plan-quality/review **judgment** via `src/shared/central-reasoning-delegation.md`; the plan artifact lifecycle stays with Effective Flow. |
...
- **`codebase-improvement` ↔ `generic-product-reviewer`:** The skill owns reusable general
  audit reasoning: evidence quality, impact, candidate rejection, prioritization, complexity,
  root cause, and scope control. The worker retains the reduced-depth notice, repository-native
  discovery order, read-only boundary, finding fields, confidence threshold, language-depth
  limitation, and Effective Flow routing. Its inline fallback is deliberately minimal.
