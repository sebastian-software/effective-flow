...
9. `prReview` – the **merge gate** of `effective-flow pr-review`, asked as its own block: see below.
...
- Ask each time -- prReview.completion = ask (default) — a gated run asks once per pull request
- Merge -- prReview.completion = merge — merge as soon as every precondition holds
- No merge -- prReview.completion = report — the gate still repairs failing checks and answers bot threads, it only never merges
