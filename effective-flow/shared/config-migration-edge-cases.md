...
per-checkout configuration that effective-flow setup alone writes. It is read with the same table
...
defaults, or let effective-flow setup write tracked configuration over a hidden main checkout. The
...
equivalent to the current one on read, and effective-flow setup converts it non-destructively to the
...
- `delivery.baseBranch`, `delivery.branchPrefix` and `delivery.completion`: effective-flow build,
  effective-flow fix, effective-flow docs, effective-flow refactor and effective-flow maintain, through "Delivery and
  worktree integration".
- `delivery.baseBranch` and `delivery.branchPrefix`: effective-flow deliver, `tools/apply-issues.md` and
  `tools/apply-review.md` in remote mode.
- `delivery.baseBranch` only: effective-flow pr, and effective-flow iterate in local mode, the only mode that
  reads it.
- every `mergeGate.*` key: effective-flow merge-gate.
- `mergeGate.bots`, `mergeGate.bots.<login>.trigger`, `mergeGate.bots.<login>.check` and
  `mergeGate.botWaitMinutes`: effective-flow iterate in PR mode, including a run effective-flow merge-gate
...
- **Successor absent → stop.** Name the retired row, its successor, and effective-flow setup, which
...
- **Successor present → the successor wins.** Report the inert retired row **once** per run, point to
  effective-flow setup, and continue; the two rows are never combined.
- **Login-keyed subkeys.** A retired `prReview.bots.<login>.trigger` or `.check` has as its successor
  ...
  and do not stop.
- **effective-flow deliver and `worktree.completion`.** `deliver` reads `delivery.completion` only to state
  that its own pull-request intent overrides it, so a retired `worktree.completion` there is reported
  and never stops the run.
- **effective-flow setup is exempt.** It is the repair path: it records every retired row and rewrites it
