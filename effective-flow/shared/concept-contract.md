...
- `<concept.dir>` and `<plan.dir>` must be **separate directories**, compared as canonical paths
  ...
  guessing, and refer the user to `effective-flow setup`.
...
- Not elaborated yet (see `effective-flow review <concept-file>`).
...
- While the status is `Draft`, the section carries exactly one bullet: the empty state from the
  template above, which names the re-entry `effective-flow review <concept-file>`. The German form reads
...
- The handoff is **self-contained text**: a complete `effective-flow plan` call whose requirement string
