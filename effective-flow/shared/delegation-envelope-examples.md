...
Three canonical delegation messages from `effective-flow merge-gate` to `effective-flow iterate`, one per
...
lines, which `effective-flow iterate` Phase 0 recognizes by their literal form alone. Without a boundary,
...
landed below the delimiter, and `effective-flow iterate` rightly answered
