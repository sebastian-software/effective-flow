...
This fragment carries the body of Phase 5.5: steps 1 to 7 and the transition gate they end in, and,
under its closing heading, the per-issue items Phase 6 reports for what those steps observed. It
loads at the moment Phase 5.5 begins, and what decides that moment stays in
`effective-flow merge-gate`'s always-loaded core – the entry condition requiring a fresh read that proves
...
The lifecycle receipt result stays in Phase 6 of the core for the same reason: a merge without a
valid receipt ends Phase 5.5 before this fragment loads, and still owes that report.
...
3. **Assess completion, without asking.** This assessment is not gated: it runs without asking, for
   ...
   **Observe the issue's recorded open points as well — for the report, and for nothing else.** The
   ...
   comment `effective-flow plan-issue` writes, which keeps its open points — the implementation-blocking
   ...
   `<!-- firmo-plan-issues -->`, exactly as `effective-flow plan-issue` selects it; every other comment
   ...
   reason: `effective-flow plan-issue` keeps an older comment written before those sections existed
...
After a confirmed merge, Phase 6 step 2 of `effective-flow merge-gate` reports these items in its chat
summary, directly after the lifecycle receipt result, for the linked issues this phase observed:

- one row per linked issue with its
...
- **per linked issue, the completion verdict** of Phase 5.5 by its name, for every issue step 3
  ...
  pull-request text: this item reads **no body** for the same reason Phase 6's guard-exclusion item
  reads none, and the operator reads each criterion at the issue and pull-request URLs. Then, per issue:
