...
This fragment is loaded only for `effective-flow setup` with no argument or the explicit `profile`
...
need integration details, point to explicit `effective-flow setup express` when the caller intentionally
...
Ask the user: **Which language should Effective Flow use in this chat?**
- Mirror -- Mirror the user's recognizable language and remove any language.chat row on the confirmed write
- English -- Use English from the next question onward and persist language.chat = en on the confirmed write
- German -- Use German from the next question onward and persist language.chat = de on the confirmed write
...
`effective-flow plan` request without an issue reference still creates a local plan file under
...
named branch or use `effective-flow setup guided`. On an unborn branch, outside Git, or where no valid
...
before writing and direct the user to `effective-flow setup guided`. A missing origin, failed
...
**Load on demand:** Read `shared/tracker-target.md`, when the selected Profile is External + forge and the external integration interview begins.
...
`effective-flow setup` asks its Visibility question after the two common questions and before Step 1.
