...
in the tool source, not inside another fragment: `effective-flow build`, `effective-flow fix`, `effective-flow docs`
and `effective-flow refactor` are the workflows that keep a plan file, and each names this fragment in its
own deferred-include fence, with a load trigger covering both the handback delivery point and
in-place execution. `effective-flow iterate`, `effective-flow maintain` and `effective-flow merge-gate` keep no plan
...
the other. The invocation shape is the probe contract `effective-flow plan` already applies to the reverse
